...
@torch.jit.script
def compute_coord(
    torso_rotation, inv_start_rot, vec0, vec1
):
    #compute the coordinate system projected on the plane
    ...
    vec_rot = torch.cross(heading_vec, vec0)
    #print('vec0:{}'.format(vec0))
    #print('heading vec:{}'.format(heading_vec))
    axis = normalize(vec_rot)
    dangle = torch.norm(vec_rot, dim = -1)
    dangle = torch.arcsin(dangle)
    quat = quat_from_angle_axis(dangle, axis)
    return quat
